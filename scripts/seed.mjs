...
const MONGODB_URI = 'mongodb://127.0.0.1:27017/bumisehat'
...
async function seed() {
    try {
        ...
        // Insert sample tips
        const createdTips = await Tips.insertMany(sampleTips)
        console.log(`✅ ${createdTips.length} tips berhasil dibuat:`)
        createdTips.forEach(tip => {
            console.log(`   - ${tip.judul} (${tip.kategori})`)
        })
